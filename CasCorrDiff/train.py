                    ...
                    for n_i in range(num_accumulation_rounds):
                        with nvtx.annotate(
                            ...
                            with nvtx.annotate("loading data", color="green"):
                                ...
                                # Unpack with mask only if dwd_mask is in static channels
                                if hasattr(dataset, 'static_channels_list') and 'dwd_mask' in dataset.static_channels_list:
                                    img_clean, img_lr, dwd_mask, *lead_time_label = data_batch
                                else:
                                    img_clean, img_lr, *lead_time_label = data_batch
                                    dwd_mask = None
                                if use_apex_gn:
                            ...
                            for patch_num_per_iter in patch_nums_iter:
                                ...
                                # Apply spatial mask to loss (if available)
                                if dwd_mask is not None:
                                    # dwd_mask may already be a tensor from the dataloader
                                    if isinstance(dwd_mask, torch.Tensor):
                                        dwd_mask_tensor = dwd_mask.to(dist.device).float()
                                    else:
                                        dwd_mask_tensor = torch.from_numpy(dwd_mask).to(dist.device).float()
                                    
                                    # Expand mask to match loss shape: (H, W) -> (B, 1, H, W)
                                    if dwd_mask_tensor.ndim == 2:
                                        dwd_mask_tensor = dwd_mask_tensor.unsqueeze(0).unsqueeze(0)
                                    elif dwd_mask_tensor.ndim == 3:
                                        dwd_mask_tensor = dwd_mask_tensor.unsqueeze(1)
                                    loss = loss * dwd_mask_tensor
                        ...
                        if is_time_for_periodic_task(
                            ...
                            with torch.no_grad():
                                for _ in range(cfg.training.io.validation_steps):
                                    data_batch_valid = next(validation_dataset_iterator)
                                    # Unpack with mask only if dwd_mask is in static channels
                                    if hasattr(validation_dataset, 'static_channels_list') and 'dwd_mask' in validation_dataset.static_channels_list:
                                        img_clean_valid, img_lr_valid, dwd_mask_valid, *lead_time_label_valid = data_batch_valid
                                    else:
                                        img_clean_valid, img_lr_valid, *lead_time_label_valid = data_batch_valid
                                        dwd_mask_valid = None
                                    ...
                                    for patch_num_per_iter in patch_nums_iter:
                                        ...
                                        # Apply spatial mask to validation loss (if available)
                                        if dwd_mask_valid is not None:
                                            # dwd_mask_valid may already be a tensor from the dataloader
                                            if isinstance(dwd_mask_valid, torch.Tensor):
                                                dwd_mask_tensor_valid = dwd_mask_valid.to(dist.device).float()
                                            else:
                                                dwd_mask_tensor_valid = torch.from_numpy(dwd_mask_valid).to(dist.device).float()
                                            
                                            # Expand mask to match loss shape: (H, W) -> (B, 1, H, W)
                                            if dwd_mask_tensor_valid.ndim == 2:
                                                dwd_mask_tensor_valid = dwd_mask_tensor_valid.unsqueeze(0).unsqueeze(0)
                                            elif dwd_mask_tensor_valid.ndim == 3:
                                                dwd_mask_tensor_valid = dwd_mask_tensor_valid.unsqueeze(1)
                                            loss_valid = loss_valid * dwd_mask_tensor_valid
