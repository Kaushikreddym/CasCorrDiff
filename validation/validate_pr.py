"""
Precipitation (pr) validation script.

Validates multiple precipitation models against GHCNd station observations:
- ERA5 (100 km input)
...
from validation.viz_utils import (
    ...
    plot_2d_density_time_means
)

# Configuration
BASE_DIR = Path(__file__).parent.parent
DATA_PATH = BASE_DIR / "station_extracts_pr/all_stations_pr_2020-2024.nc"
OUTPUT_DIR = BASE_DIR / "validation/outputs/pr"
...
# Load data
print("\nLoading precipitation station data...")
...
print(f"  10km prediction mean: {pred_10km_mean.mean().values:.2f} mm/day")
print(f"  ISIMIP prediction mean: {pred_isimip_mean.mean().values:.2f} mm/day")
print(f"  1km prediction mean: {pred_1km_mean.mean().values:.2f} mm/day")
...
plot_seasonal_maps(datasets, col_titles, ds_obs, 
                  output_path=OUTPUT_DIR / "seasonal_maps_pr.png",
                  variable_name='pr', variable_units='mm/day')
print(f"Saved: {OUTPUT_DIR / 'seasonal_maps_pr.png'}")
...
# Calculate metrics (wet threshold = 1.0 mm/day for precipitation)
print("Calculating metrics for each model...")
metrics_era5 = calculate_metrics(obs.values, era5_input_aligned.values, wet_threshold=1.0)
metrics_10km = calculate_metrics(obs.values, pred_10km_aligned.values, wet_threshold=1.0)
metrics_isimip = calculate_metrics(obs.values, pred_isimip_aligned.values, wet_threshold=1.0)
metrics_1km = calculate_metrics(obs.values, pred_1km_aligned.values, wet_threshold=1.0)
...
all_metric_info = [
    {'name': 'RMSE (mm/day)', 'key': 'rmse', 'cmap': cmaps.agsunset_r},
    {'name': 'Spearman ρ', 'key': 'spearman', 'cmap': cmaps.agsunset_r, 
     'vmin': 0.6, 'vmax': 0.95},
    {'name': 'Wasserstein Distance (mm/day)', 'key': 'wasserstein', 
     'cmap': cmaps.agsunset_r},
    {'name': 'Frequency Bias', 'key': 'freq_bias', 'cmap': cmaps.temps, 
     'center': 1.0, 'vmin': 0.5, 'vmax': 1.5},
    {'name': 'Probability of Detection (POD)', 'key': 'pod', 
     'cmap': cmaps.agsunset_r, 'vmin': 0.5, 'vmax': 1.0},
    {'name': 'Critical Success Index (CSI)', 'key': 'csi', 
     'cmap': cmaps.agsunset_r, 'vmin': 0.5, 'vmax': 0.8},
    {'name': 'Extreme Bias (mm/day)', 'key': 'extreme_bias', 
     'cmap': cmaps.precip_diff_12lev, 'center': 0.0}
]

for metric_info in all_metric_info:
    metric_key = metric_info['key']
    output_file = OUTPUT_DIR / f"metric_{metric_key}_pr.png"
...
plot_quantile_difference(obs, model_data_list, 
                        output_path=OUTPUT_DIR / "quantile_difference_pr.png",
                        variable_name='pr', variable_units='mm/day')
print(f"Saved: {OUTPUT_DIR / 'quantile_difference_pr.png'}")
...
plot_2d_density_scatter(obs, model_data_list_scatter,
                       output_path=OUTPUT_DIR / "density_scatter_pr.png",
                       variable_name='pr', variable_units='mm/day')
plot_2d_density_time_means(obs, model_data_list_scatter,
                       output_path=OUTPUT_DIR / "density_scatter_pr_timemean.png",
                       variable_name='pr', variable_units='mm/day')
print(f"Saved: {OUTPUT_DIR / 'density_scatter_pr.png'}")

print("\n" + "="*80)
print("PRECIPITATION VALIDATION COMPLETE")
print("="*80)
